...
class Tile {
  ...
  copy() {
    const temp = new Tile({
      x: this.x,
      y: this.y,
      ocupide: this.ocupide,
      clutterd: this.clutterd,
    });
    for (let i = 0; i < 4; i++) {
...
class Board {
  ...
  shortestPath(startPoint, endPoint) {
    ...
    while (posiblePaths[0] != undefined) {
      ...
      if (
        (finishedPaths[0] != undefined &&
          posiblePaths[0].traveled < finishedPaths[0].traveled) ||
        finishedPaths[0] === undefined
      ) {
          ...
          if (rootTile.walls[c] === false) {
            ...
            if (save && lenthList.testLegnths(list, virtualPath)) {
              if (virtualCo.same(endPoint)) {
                if (
                  finishedPaths[0] != undefined &&
                  virtualPath.traveled < finishedPaths[0].traveled
                ) {
                  finishedPaths = [];
                }
  ...
  makeGoal() {
    ...
    switch (this.goals.length) {
      case 0:
        ...
        this.goals.push(
          new Goal(this.randomFromQuadrent(quad), quad, "cherry")
        );
...
class Path {
  ...
  findCo(co1) {
    // takes Co
    let found = null;
    this.tiles.forEach((co2, index) => {
      found = co1.same(co2) ? index : found;
    });
    return found;
  } //tested
...
class ViewTerminal {
  drawWallsThick(board) {
    ...
    for (let l = 0; l < boardSize; l++) {
      draw.push(["", "", "", "", ""]);
      for (let w = 0; w < boardSize; w++) {
        board.findI(l, w).walls[0]
          ? (draw[l][0] += verWall)
          : (draw[l][0] += verEdge);
        board.findI(l, w).walls[2]
          ? (draw[l][4] += verWall)
          : (draw[l][4] += verEdge);
        board.findI(l, w).walls[3]
          ? (draw[l] = this.drawThree(draw[l], westWall))
          : (draw[l] = this.drawThree(draw[l], westEdge));
        board.findI(l, w).walls[1]
          ? (draw[l] = this.drawThree(draw[l], eastWall))
          : (draw[l] = this.drawThree(draw[l], eastEdge));
      }
    }
...
class Wall {
  constructor(wallArray) {
    ...
    this.id =
      ...
      this.array[2] +
      "-" +
      this.array[3]; //string id
  } // Tested

  copy() {
    return new Wall(this.array);
  } // Tested

  onBoard() {
    let result =
      this.array[0] < 0 ||
      (this.array[0] > boardSize - 2 && this.vertical !== true) ||
      (this.array[0] > boardSize - 1 && this.vertical == true)
        ? false
        : true;
    result =
      this.array[1] < 0 ||
      (this.array[1] > boardSize - 1 && this.vertical !== true) ||
      (this.array[1] > boardSize - 2 && this.vertical == true)
        ? false
        : result;
    return result;
  } //tested
...
class ViewHtml {
  ...
  drawTiles(board) {
    ...
    board.tiles.forEach((tile) => {
      ...
      this.boardPlayable.insertAdjacentHTML(
        "beforeend",
        this.addEatMe(tile.co)
      );
    });
  } // tested
    ...
    amount.forEach((directions, index) => {
      for (let a = 0; a < 4; a++) {
        if (directions[a] !== 0) {
          ...
          this.boardPlayable.insertAdjacentHTML(
            "beforeend",
            this.addPath(map.tiles[index], a, inAll)
          );
        }
      }
    });
  ...
  drawControler() {
    if (innerWidth > innerHeight) {
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="btnSizeDouble btn">PACK TACK</div>'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="horizontalSort" id="pathsControler">'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="btnSizeDouble btn" id="movesControler"></div>'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="btnSize btn" id="resetControler">Reset</div>'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="btnSize btn" id="colectedControler"></div>'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="btnSizeDouble btn" id="goControler">Go</div>'
      );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="horizontalSort" id="statsControler">'
      );
      document
        .querySelector("#statsControler")
        .insertAdjacentHTML(
          "beforeend",
          '<div class="btnSize btn" id="totalMovesControler"></div>'
        );
      document
        .querySelector("#statsControler")
        .insertAdjacentHTML(
          "beforeend",
          '<div class="btnSize btn" id="turnsControler"></div>'
        );
      ...
      document
        ...
        .insertAdjacentHTML(
          "afterbegin",
          '<div class="titleControlerControlerSize" id="titleControler">'
        );
      document
        .querySelector("#titleControler")
        .insertAdjacentHTML(
          "afterbegin",
          '<div class="btnSizeDouble btn">PACK TACK</div>'
        );
      ...
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="horizontalSort" id="v1Controler">'
      );
      document
        .querySelector("#v1Controler")
        .insertAdjacentHTML(
          "beforeend",
          '<div class="btnSizeDouble btn" id="goControler">Go</div>'
        );
      document
        ...
        .insertAdjacentHTML(
          "beforeend",
          '<div class="btnSizeDouble btn" id="resetControler">Reset</div>'
        );
      this.controler.insertAdjacentHTML(
        "beforeend",
        '<div class="horizontalSort" id="v2Controler">'
      );
      document
        .querySelector("#v2Controler")
        .insertAdjacentHTML(
          "beforeend",
          '<div class="btnSize btn" id="movesControler"></div>'
        );
      document
        .querySelector("#v2Controler")
        .insertAdjacentHTML(
  ...
  drawPack(board) {
    ...
    this.boardPlayable.insertAdjacentHTML(
      "beforeend",
      `<img src="./modules/pack.svg" id="pack" class="pack row${board.pack.x} column${board.pack.y} tileSize">`
    );
  }
  ...
  addPath(drawPathCo, rotate, shouldHilight) {
    return `<img src="./modules/path.svg" id=${
      "path-" + drawPathCo.id + "-" + rotate
    } class="path row${drawPathCo.x} column${
      drawPathCo.y
    } rotate${rotate} tileSize">`;
  } // tested but hilighting options needed

  updateControler(board) {
    this.removeClass("pathControler");
    this.pathsControler.insertAdjacentHTML(
      "beforeend",
      `<div class="btnSize btn pathControler" id="pathControler${0 + 1}">${
        0 + 1
      }</div>`
    );
  ...
  updateGoals(board) {
    ...
    board.goals.forEach((goal) => {
      ...
      this.boardPlayable.insertAdjacentHTML(
        "beforeend",
        `<img src="./modules/${goal.name}.svg" id="goal${goal.co.id}" class="goal row${goal.co.x} column${goal.co.y} tileSize">`
      );
    });
  }
  ...
  packAnimate(co, dir) {
    ...
    interval = setInterval((frame) => {
      console.log(posY);
      if (
        (posX >= finalX && posY >= finalY && (dir == 1 || dir == 2)) ||
        (posX <= finalX && posY <= finalY && (dir == 0 || dir == 3))
      ) {
        clearInterval(interval);
        master.packMove();
      } else {
  ...
  setCssSizes() {
    ...
    let halfHilighWidthDiference = Math.floor(
      ((hilightScale - 1) / 2) * tileSize
    ); //ajust for rounding?
    ...
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.boardPlayableSize{width: ${boardMax}px; height: ${boardMax}px;}`
    );
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.tileSize{width: ${tileSize}px;}`
    );
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.wallSize{height: ${tileSize}px;}`
    );
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.WallHilightSizeH{width: ${wallHiglightSize}px;}`
    );
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.WallHilightSizeV{height: ${wallHiglightSize}px;}`
    );

    for (let g = 0; g < boardSize; g++) {
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.row${g}{top: ${tileSize * g}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.column${g}{Left: ${tileSize * g}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.rowWall${g}{top: ${tileSize * (g + 1) - halfWallWidth}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.columnWall${g}{Left: ${tileSize * (g + 1) - halfWallWidth}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.rowHilight${g}{top: ${tileSize * g - halfHilighWidthDiference}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.columnHilight${g}{Left: ${
          tileSize * g - halfHilighWidthDiference
        }px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.rowWallHilight${g}{top: ${
          tileSize * (g + 1) - halfHilighHeightDiference
        }px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.columnWallHilight${g}{Left: ${
          tileSize * (g + 1) - halfHilighHeightDiference
        }px;}`
      );
    }
    ...
    if (screenHoizontal) {
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.mainControlerSize{width: ${boardMax / 2}px; height:${boardMax}}`
      );
    } else {
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.mainControlerSize{width: ${boardMax}px; height:${boardMax / 2}}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.titleControlerControlerSize{width: ${boardMax}px;}`
      );
      this.style.insertAdjacentHTML(
        "afterbegin",
        `.mainContainerDir{flex-direction:column;}`
      );
    }

    this.style.insertAdjacentHTML(
      "afterbegin",
      `.btnSize{font-size: ${Math.floor(boardMax / 20)}px; height:${Math.floor(
        btnSize - 4
      )}px}`
    );
    this.style.insertAdjacentHTML(
      "afterbegin",
      `.btnSizeDouble{font-size: ${Math.floor(
        boardMax / 14
      )}px; height:${Math.floor(btnSize * 2 - 4)}px}`
    );
...
class Gamemaster {
  ...
  selectAnyWall() {
    ...
    this.shortestPaths = this.gameBoard.shortestPath(
      this.gameBoard.pack,
      this.gameBoard.goals[0].co
    );
  ...
  ChangeWall(wallId) {
    ...
    let newWall = new Wall([
      Math.floor(araryOfStrings[0]),
      Math.floor(araryOfStrings[1]),
      Math.floor(araryOfStrings[2]),
      Math.floor(araryOfStrings[3]),
    ]);
    console.log(newWall);
    ...
    this.view.boardPlayable.insertAdjacentHTML(
      "beforeend",
      this.view.addWall(newWall)
    );
  ...
  packMove() {
    ...
    if (this.movePostion === path.tiles.length - 1) {
      ...
      this.gameBoard.movePack(path.tiles[this.movePostion]);
      let dir = path.tiles[this.movePostion].direction(
        path.tiles[this.movePostion + 1]
      );
